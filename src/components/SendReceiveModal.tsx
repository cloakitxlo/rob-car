    ...
    <div className="fixed inset-0 z-50 bg-black/55 backdrop-blur-md flex items-center justify-center p-4">
      <div className="glass-card border border-white/10 rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center border-b border-white/10 pb-4">
          <div className="flex items-center gap-1 bg-[#1c180d]/80 p-1 rounded-xl border border-white/10">
            <button
              onClick={() => {
                setMode('send');
                setErrorMsg(null);
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${
                mode === 'send' ? 'trust-gradient shadow-md' : 'text-[#a09c8f] hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${
                mode === 'receive' ? 'trust-gradient shadow-md' : 'text-[#a09c8f] hover:text-white'
              }`}
            >
        ...
        {scannedNotice && (
          <div className="p-3 rounded-2xl bg-emerald-500/15 border border-[#ccff00]/30 text-[#ccff00] text-xs font-bold flex items-center gap-2 animate-fade-in">
        ...
        {sendSuccess ? (
          <div className="text-center py-6 space-y-3">
            <div className="w-14 h-14 rounded-full bg-[#ccff00]/10 border border-[#ccff00]/30 text-[#ccff00] flex items-center justify-center mx-auto shadow-lg shadow-emerald-500/10">
              <Check className="w-7 h-7 text-[#ccff00]" />
            </div>
            <h4 className="text-base font-semibold text-[#faf7f0]">Asset Sent Successfully!</h4>
          ...
          <form onSubmit={handleSendSubmit} className="space-y-4">
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-left transition-all ${
                      selectedAsset.id === a.id
                        ...
                        : 'bg-black/30 border-white/5 text-[#e8e5dc] hover:bg-[#35322d]'
                    }`}
                  >
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="text-xs font-semibold text-[#e8e5dc] uppercase tracking-wider block">
                  Recipient Wallet Address
                </label>
                <button
                  type="button"
                  onClick={() => setShowScanner(true)}
                  className="text-[11px] font-bold text-[#ccff00] hover:text-[#ccff00] flex items-center gap-1 hover:underline transition-all"
                >
              ...
              <input
                ...
                className="w-full glass-input rounded-xl px-3.5 py-2.5 text-xs font-mono text-[#faf7f0] focus:outline-none"
              />
            </div>
            ...
            <div className="bg-black/55 p-4 rounded-2xl border border-white/10 space-y-2">
              <div className="flex justify-between items-center text-xs">
                <span className="text-[#a09c8f] font-bold">Transfer Amount ({selectedAsset.symbol})</span>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-[#ccff00]/15 hover:bg-[#ccff00]/30 text-[#ccff00] text-xs font-semibold uppercase border border-[#ccff00]/30"
                >
                  MAX
                </button>
              </div>

              <div className="flex justify-between items-center pt-2 border-t border-white/5 text-xs font-mono text-[#a09c8f]">
                <span>Estimated Value:</span>
                <span className="font-bold text-[#e8e5dc]">
            ...
            <button
              ...
              className={`w-full py-3.5 rounded-full font-semibold text-xs transition-all shadow-lg flex items-center justify-center gap-2 ${
                numericAmt <= 0
          ...
          <div className="space-y-5 text-center">
            ...
            <div className="p-3.5 rounded-2xl bg-[#ccff00]/10 border border-[#ccff00]/25 text-xs text-[#e8e5dc] flex items-center justify-between">
              <div className="flex items-center gap-2 font-semibold text-[#ccff00]">
                <ShieldCheck className="w-4 h-4" />
                <span>Deposit Network: USDT (TRC20)</span>
              </div>
              <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-[#ccff00]/15 text-[#ccff00] border border-[#ccff00]/30 uppercase">
                Exclusive Method
              </span>
            </div>

            {/* Real QR Code graphic corresponding to static TRC20 address */}
            <div className="p-4 rounded-2xl bg-white text-slate-950 inline-block shadow-2xl border-4 border-[#ccff00]/30">
              ...
              <span className="text-[10px] font-semibold font-mono text-slate-900 mt-2 block uppercase tracking-wider">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <p className="text-xs font-semibold text-[#e8e5dc] uppercase tracking-wider text-left">
                  Official TRC20 Deposit Address
                </p>
                {copied && (
                  <span className="text-[10px] font-semibold text-[#ccff00] bg-[#ccff00]/10 px-2 py-0.5 rounded-full border border-[#ccff00]/25 animate-in fade-in">
                    Address Copied to Clipboard!
                  </span>
                )}
              </div>
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 p-3 rounded-2xl bg-[#0a0805]/90 border border-white/10 font-mono text-xs text-[#e8e5dc] shadow-inner">
                <span className="truncate font-semibold text-amber-400 select-all self-center text-center sm:text-left px-1">
                  {activeDepositAddress}
                </span>
                <button
                  type="button"
                  onClick={handleCopy}
                  className={`px-4 py-2 rounded-full text-xs font-semibold transition-all flex items-center justify-center gap-2 shrink-0 border shadow-md active:scale-95 ${
                    copied
                      ? 'bg-[#ccff00]/15 border-emerald-500 text-[#ccff00] shadow-emerald-500/20'
                      : 'trust-gradient border-[#ccff00] hover:trust-gradient-hover shadow-[#ccff00]/15'
                  }`}
            ...
            <button
              type="button"
              onClick={() => setShowScanner(true)}
              className="w-full py-2.5 px-3 rounded-2xl bg-[#ccff00]/15 hover:bg-[#ccff00]/25 border border-[#ccff00]/30 text-[#ccff00] font-semibold text-xs transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95 group"
            >
              <Camera className="w-4 h-4 text-[#ccff00] group-hover:scale-110 transition-transform" />
            ...
            <div className="text-left space-y-2 pt-1">
              <div className="flex justify-between items-center text-xs">
                <label className="font-semibold text-[#e8e5dc] uppercase tracking-wider block">
                  Deposit Amount (USDT)
                </label>
                ...
                {['11', '50', '100', '250', '500', '1000'].map((val) => (
                  <button
                    ...
                    className={`py-1.5 rounded-lg text-[10px] font-mono font-bold border transition-all ${
                      simAmount === val
                        ? 'bg-[#ccff00] text-[#110e08] border-[#ccff00] shadow-md'
                        : 'bg-[#1c180d] text-[#e8e5dc] border-white/10 hover:bg-[#35322d]'
                    }`}
                  >
                    ${val}
                  </button>
            ...
            <div className="text-left space-y-1.5 pt-1">
              <div className="flex justify-between items-center gap-2 flex-wrap">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="text-[10px] text-[#ccff00] hover:text-[#ccff00] font-mono font-bold underline flex items-center gap-1"
                  >
              ...
              {showHashHelp && (
                <div className="rounded-2xl border border-amber-500/25 bg-amber-500/5 p-3.5 space-y-2.5 text-left">
                  ...
                  <ol className="list-decimal pl-4 space-y-1.5 text-[11px] text-[#e8e5dc] leading-relaxed">
                    <li>
                    ...
                    <li>
                      Tap <span className="text-white font-bold">Copy</span>, then come back here and click <span className="text-[#ccff00] font-bold">Paste Hash</span>.
                    </li>
                  </ol>
                  <div className="pt-1 border-t border-white/10 space-y-1">
                    <p className="text-[10px] text-[#a09c8f] leading-relaxed">
                      <span className="text-[#e8e5dc] font-bold">Trust Wallet / MetaMask:</span> Wallet → History / Activity → open the send → Copy transaction hash.
                    </p>
                    <p className="text-[10px] text-[#a09c8f] leading-relaxed">
                      <span className="text-[#e8e5dc] font-bold">Exchange (Binance, etc.):</span> Wallet → Withdrawal history → open the withdrawal → copy TxID / TxHash.
                    </p>
                    <p className="text-[10px] text-[#a09c8f] leading-relaxed">
              ...
              <input
                ...
                className={`w-full bg-black/55 border ${
                  !customTxHash.trim()
                    ? 'border-amber-500/40 bg-amber-500/5'
                    : /^(0x)?[a-fA-F0-9]{64}$/.test(customTxHash.trim())
                    ? 'border-[#ccff00]/40'
                    : 'border-[#ff5000]/40 bg-rose-500/5'
                } rounded-2xl p-3 text-xs text-[#e8e5dc] font-mono placeholder:text-slate-600 focus:outline-none focus:border-[#ccff00] transition-all`}
              />
              <p className="text-[10px] text-[#a09c8f] font-mono">
            ...
            {/* Error Banner */}
            {receiveErrorMsg && (
              <div className="p-3.5 rounded-2xl bg-[#ff5000]/15 border border-[#ff5000]/40 text-[#ffb39a] text-xs font-bold text-left flex items-start gap-2.5 animate-fade-in shadow-lg shadow-rose-500/10">
                <AlertCircle className="w-4 h-4 text-[#ff5000] shrink-0 mt-0.5" />
            ...
            <div className="pt-2 border-t border-white/10">
              <button
                ...
                className="w-full py-3.5 rounded-full trust-gradient hover:trust-gradient-hover text-xs font-semibold transition-all shadow-lg flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50"
              >
