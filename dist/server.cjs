...
function generateUserCard(user) {
  ...
  const cardHolderName = user.name && user.name.trim() ? user.name.trim() : "Robin Card Member";
  return {
    id: `card-${user.id}`,
    cardNumber,
...
async function startServer() {
  ...
  app.get("/api/health", (req, res) => {
    res.json({ status: "ok", service: "Robin Card API", version: "2.0.0" });
  });
  ...
  app.post("/api/auth/login", (req, res) => {
    ...
    res.json({
      ...
      user: {
        id: matchedUser.id,
        email: cleanEmail,
        name: matchedUser.name || "Robin Card User",
        role: "user",
  ...
  app.post("/api/auth/update-profile", (req, res) => {
    ...
    res.json({
      ...
      user: {
        ...
        name: name || (targetUser ? targetUser.name : "Robin Card Member"),
        role: "user",
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Robin Card server running on http://0.0.0.0:${PORT}`);
  });
}
startServer();
